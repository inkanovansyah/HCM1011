...
class ListSatuanList {
  final String baseUrl = "https://api.1011.co.id";

  Future<ListSatuan> factchSatuan() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      var company_id = prefs.getString('company_id');
      var token = prefs.getString('token');

      final Uri url = Uri.parse('$baseUrl/kpi/$company_id/satuan/list-option');
      ...
      if (response.statusCode == 200) {
        ...
        final modelListSatuan = ListSatuan.fromJson(decodedResponse);
        // final modelListInfoString = json.encode(modelListSatuan);
        // print('ModelListInfo as String: ${modelListInfoString}');
        return modelListSatuan;
      } else {
